...
namespace BioEngineerLab.Core
{
    public class CraftService : IService
    {
        ...
        public void HeatStir(Container container)
        {
            ...
            if(TryFindCraftBySubstances(container.SubstancesList, ECraft.HeatStir, out CraftConfig craft))
            {
                return;
            }
            ...
            var resSubstanceProperty = FindSubstancePropertyByMode(heatingSubstance.SubstanceProperty.SubstanceName, ESubstanceMode.HeatStir);
        ...
        private bool TryFindCraftBySubstances(IReadOnlyCollection<Substance> substances, ECraft eCraft, out CraftConfig craftConfig)
        {
            ...
            foreach (var craft in Configuration.Crafts)
            {
                if (craft.CraftType == eCraft & substances.Count == craft.SubstancesFrom.Length && craft.SubstancesFrom.All(substances.Contains))
                {
                    craftConfig = craft;
                    return true;
                }
        ...
        public void Dry(Container container)
        {
            ...
            var resSubstanceProperty = FindSubstancePropertyByMode(dryingSubstance.SubstanceProperty.SubstanceName, ESubstanceMode.Dry);
        ...
        private SubstanceProperty FindSubstancePropertyByMode(ESubstanceName name, ESubstanceMode mode)
        {
            ...
            {
                if (substanceProperty.eSubstanceName == name
                     && substanceProperty.eSubstanceMode == mode)
                {
        ...
        private SubstanceProperty FindMixSubstanceProperty(SubstanceProperty sub1, SubstanceProperty sub2)
        {
            ...
            foreach (var substanceProperty in Configuration.SubstanceProperties)
            {
                isSub1Find = false;
                IsSub2Find = false;
                if (substanceProperty.eSubstanceMode != ESubstanceMode.Mix || substanceProperty.SubSubstanceProperties.Count != 2)
                {
                    continue;
                }
